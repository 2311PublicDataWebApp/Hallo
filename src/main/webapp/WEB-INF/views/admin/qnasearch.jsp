...
<body>
							...
							<table class="table table-hover">
								...
								<c:choose>
										<c:when test="${fn:length(searchList) !=0 }">
											<c:forEach items="${searchList }" var="qna" varStatus="i">
								...
								<div class="d-flex flex-wrap justify-content-center align-items-center pb-5">
				                    <div class="d-flex col-md-6 justify-content-start">
				                       <form class="row g-1" action="/admin/qnasearch.do" name="adm_form" method="get">
										<div class="col-auto">
											<select class="form-select" name="searchCondition"  id="searchqnacon">
												...
												<option value="qnaStatus" 	<c:if test="${searchCondition == 'qnaStatus'}">selected</c:if>>상태</option>
												<option value="qnaTitle"	<c:if test="${searchCondition == 'qnaTitle'}">selected</c:if>>제목</option>
												<option value="qnaDate" 	<c:if test="${searchCondition == 'qnaDate'}">selected</c:if>>작성일시</option>
											</select>
										</div>
										<div class="col-auto">
										...
										<nav aria-label="Page navigation example">					
											<ul class="pagination justify-content-center" style="font-weight: 600; ">
												<c:if test="${pInfo.startNavi ne '1' }">
													<li class="page-item">
								                       <a class="page-link rounded-circle" href="/admin/qnasearch.do?page=${pInfo.startNavi - 1 }" aria-label="Previous">
												...
												<c:forEach begin="${pInfo.startNavi }" end="${pInfo.endNavi }" var="p">
													 <li class="page-item">
					                                     <a class="page-link rounded-circle mx-2" href="/admin/qnasearch.do?page=${p }" style="border: none; color: #313131;">
					                                         ${p }
					                                     </a>
				                                 	</li>
												</c:forEach>
												<c:if test="${pInfo.endNavi ne pInfo.naviTotalCount }">
													<li class="page-item">
					                                     <a class="page-link rounded-circle" href="/admin/qnasearch.do?page=${pInfo.endNavi + 1 }" aria-label="Next">
	...
	function removeWord(){
	    if( adm_form.searchKeyword.value != "" ) {
	    	...
	    	document.getElementById("searchqnacon").value = "all";
	     }
	}
